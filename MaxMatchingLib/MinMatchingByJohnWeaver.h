...
#ifndef SMART_SZX_GOAL_MIN_MATCHING_BY_JOHN_WEAVER_H
#define SMART_SZX_GOAL_MIN_MATCHING_BY_JOHN_WEAVER_H
...
struct MinMatchingByJohnWeaver { // https://github.com/saebyn/munkres-cpp
    MinMatchingByJohnWeaver(const Arr2D<int> &costMat)
        : matrix(costMat), mask_matrix(costMat.size1(), costMat.size2()), assignment(costMat.size1()) {
        mask_matrix.reset(Arr2D<int>::ResetOption::AllBits0);
    }

    const Arr<int>& solve() { // maximize the total cost.
        if (matrix.size1() > matrix.size2()) { return assignment; }
...
protected:
    ...
    void retrieveSln() {
        for (int r = 0; r < mask_matrix.size1(); ++r) {
            for (int c = 0; c < mask_matrix.size2(); ++c) {
                if (mask_matrix(r, c) == STAR) { assignment[r] = c; break; }
            }
        }
    }
    ...
    struct Matrix : public Arr2D<T, IndexType> {
        ...
        int rows() const { return Arr2D<T, IndexType>::size1(); }
        int columns() const { return Arr2D<T, IndexType>::size2(); }
        int minsize() const { return rows(); }
    };
    ...
    static void replace_infinites(Matrix<Data> &matrix) {
        if (!std::numeric_limits<Data>::has_infinity) { return; }
...
#endif // SMART_SZX_GOAL_MIN_MATCHING_BY_JOHN_WEAVER_H
